...
var EncapsulatedPacket = require('./encapsulatedpacket').EncapsulatedPacket;
...
var NetworkHandler = (function (_EventEmitter) {
  ...
  _createClass(NetworkHandler, [{
    ...
    value: function receiveMessage(rawMessage, rinfo) {
      ...
      if (isEncapsulated) {
        EncapsulatedPacket.fromBuffer(rawMessage, function (err, packet) {
    ...
    value: function sendAck() {
      ...
      Object.keys(sequences).forEach(function (client) {
        ...
        var packet = Packet.create(Protocol.ACK, { payload: list });
    ...
    value: function sendMessageTo(message, rinfo) {
      ...
      if (!this.ready) {
        console.log('Message send, but socket not ready');
        console.log('Forcing ready state');
        this.ready=true;
      }

      if (message instanceof Packet) {
        console.log('CHK: if (message instanceof Packet)');
